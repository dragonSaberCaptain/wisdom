...
public class TUserServiceImpl implements TUserService {
    ...
    public TUser selectByPrimaryKey(Long userId) {
        ...
        if (tUser.getIsDelete().equals(Global.ONE)) {
            logger.error("用户已删除");
            throw new UserException(UserEnum.DELETED);
        }
        if (tUser.getIsRegister().equals(Global.ZERO)) {
            logger.error("用户未注册");
    ...
    public List<TUser> selectBySelective(TUser tUser) {
        tUser.setIsDelete(Global.ZERO);
        tUser.setIsRegister(Global.ONE);
    ...
    public Map<String, Object> login(String mobile, String code) {
        ...
        if (tUsers.get(0).getIsDelete().equals(Global.ONE)) {
            logger.error("用户已删除");
            throw new UserException(UserEnum.DELETED);
        }
        if (tUsers.get(0).getIsRegister().equals(Global.ZERO)) {
            logger.error("用户未注册");
            throw new UserException((UserEnum.UNREGISTERED));
        }
