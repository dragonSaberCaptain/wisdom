...
public class ThumbnailatorUtil {
    ...
    public static void ImgThumb(MultipartFile imageFile, String output, int width, int height) {
        try {
            Thumbnails.of(imageFile.getInputStream()).size(width, height).outputQuality(1f).outputFormat("jpg").toFile(output);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
