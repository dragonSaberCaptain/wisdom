...
public class ExceptionHandle {
    ...
    public ResultVo handle(RuntimeException e) {
        if (e instanceof UserException) {
            ...
            return new ResultVo(e1.getCode(), e1.getMsg());
        }
    ...
    @ResponseBody
    public ResultVo handle(IOException e) {
        if (e instanceof FileNotFoundException) {
            FileNotFoundException e1 = (FileNotFoundException) e;
            logger.error("【文件路径异常】", e1);
            return new ResultVo(ResultEnum.FILE_PATH_ERROR);
        }
        logger.error("【IO异常】", e);
        return new ResultVo(ResultEnum.IO_ABNORMAL);
    }

    @ExceptionHandler(ServletException.class)
    @ResponseBody
    public ResultVo handle(ServletException e) {
        if (e instanceof MissingServletRequestParameterException) {
            MissingServletRequestParameterException e1 = (MissingServletRequestParameterException) e;
            logger.error("【请求参数异常】", e1);
            return new ResultVo(ResultEnum.PARAMS_ERROR);
        }
    ...
    @ResponseBody
    public ResultVo handle(ReflectiveOperationException e) {
        if (e instanceof NoSuchMethodException) {
            NoSuchMethodException e1 = (NoSuchMethodException) e;
            logger.error("【未知方法异常】", e1);
            return new ResultVo(ResultEnum.NO_SUCH_METHOD);
        }
        logger.error("【服务器内部异常】", e);
        return new ResultVo(ResultEnum.INTERNAL_SERVER_ERROR);
    }
