...
<mapper namespace="${packageName}.${moduleName}.mapper.${ClassName}Mapper">
    ...
    <insert id="save" parameterType="${packageName}.${moduleName}.domain.${ClassName}">
        ...
        (<#list list as column><#if column.columnName == "id"><#else>${column.typeName}<#if column_index+1 != listSize>
        ,<#else></#if></#if></#list>)values(<#list list as column><#if column.columnName == "id"><#else >
        #${leftBraces}${column.columnName},jdbcType=${column.jdbcType}${rightBraces}<#if column_index+1 != listSize>
            ,<#else></#if></#if ></#list>)
    </insert>
    ...
    <update id="update" parameterType="${packageName}.${moduleName}.domain.${ClassName}">
        update ${tableName} set
         <#list list as column>
             <#if column.columnName == "id">
             <#else >
                 ${column.typeName} = #${leftBraces}${column.columnName}
                 ,jdbcType=${column.jdbcType}${rightBraces}<#if column_index+1 != listSize>,<#else></#if>
